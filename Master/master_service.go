...
func main() {
	...
	if response.StatusCode != http.StatusOK {
		fmt.Println("Error: can't get storege address.")
		fmt.Println(response.Body)
		return
	}
	data, err = ioutil.ReadAll(response.Body)
...
func getNewTask(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		respponse, err := http.Post("http://"+databaseLocation+"/getNewTask", "text/plain", nil)
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			fmt.Fprint(w, "Error: ", err)
			return
		...
		_, err = io.Copy(w, respponse.Body)
		if err != nil {
			w.WriteHeader(http.StatusBadRequest)
			fmt.Fprint(w, "Error: ", err)
			return
...
func registerInKVStore() bool {
	...
	if response.StatusCode != http.StatusOK {
		fmt.Println("Error: Failure when contacing key-value store: ", string(data))
		return false
	}
